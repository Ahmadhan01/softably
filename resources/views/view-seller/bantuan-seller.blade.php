                    ...
                    <div id="faq-content" class="help-content-panel" style="display: none;">
                        ...
                        <div class="space-y-4 text-gray-300">
                            <div class="border-b border-gray-700 pb-2 mb-2">
                                ...
                                <p class="mt-1 text-sm text-gray-400">A: Anda dapat melacak semua pesanan Anda melalui
                                    menu "My Orders"
                            ...
                            <div class="border-b border-gray-700 pb-2 mb-2">
                                <h3 class="font-semibold text-white">Q: Bisakah saya mengembalikan produk yang sudah
                                    dibeli?</h3>
                                <p class="mt-1 text-sm text-gray-400">A: Kebijakan pengembalian dana kami bervariasi
                                    tergantung pada
                            ...
                            <div class="border-b border-gray-700 pb-2 mb-2">
                                <h3 class="font-semibold text-white">Q: Bagaimana cara menghubungi dukungan pelanggan?
                                </h3>
                                <p class="mt-1 text-sm text-gray-400">A: Anda dapat menghubungi dukungan pelanggan kami
                                    melalui
                            ...
                            <div class="border-b border-gray-700 pb-2 mb-2">
                                <h3 class="font-semibold text-white">Q: Apa perbedaan antara Aplikasi, Konten Digital,
                                    Kursus Online, dan Aset Digital?</h3>
                                <p class="mt-1 text-sm text-gray-400">A: <b>Aplikasi</b> adalah perangkat lunak yang
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Dapatkan user ID dari Blade ke JavaScript dengan aman
    // Ensure this line correctly outputs the authenticated user's ID.
    // If not logged in, it might throw an error or be null.
    const authUserId = "{{ auth()->id() }}"; // Corrected Blade syntax
    ...
    function activateTab(targetId) {
        ...
        // Mengatur scroll ke atas saat tab berubah (opsional)
        if (targetPanel) targetPanel.scrollTop = 0;
    }

    function loadAdminMessages() {
        // Only attempt to load messages if authUserId is available
        if (!authUserId) {
        ...
        fetch(`/chat/admin/messages`)
            .then(res => {
                if (!res.ok) {
                    // Check for specific status codes if needed, e.g., 401 for unauthorized
                    if (res.status === 401) {
            ...
            .then(data => {
                ...
                data.forEach(msg => {
                    ...
                    const isMine = msg.sender_id ==
                        authUserId; // Compare with the obtained authUserId
                    div.className = 'flex ' + (isMine ? 'justify-end' : 'justify-start') +
                        ' mb-2';
                    div.innerHTML =
                        `<div class="${isMine ? 'bg-blue-600 text-white' : 'bg-gray-700'} p-2 rounded-lg max-w-xs">${msg.content}</div>`;
    ...
    function sendMessageToAdmin() {
        ...
        // Only attempt to send messages if authUserId is available
        if (!authUserId) {
            alert('You must be logged in to send messages.');
            console.warn('User not authenticated. Cannot send chat messages.');
            return;
        }
        ...
        fetch('/chat/admin/send', {
            ...
            .catch(error => {
                ...
                alert('Gagal mengirim pesan. Periksa koneksi dan pastikan login.');
            });
    }

    // --- Event Listeners ---

    // Menginisialisasi tab 'Home' sebagai aktif secara default saat halaman dimuat
    ...
    // Tombol "Talk with Softably"
    talkWithSoftablyBtn.addEventListener('click', function() {
        activateTab('chat-softably-content');
        loadAdminMessages(); // Memuat pesan saat chat dibuka
    });

    // Mengirim pesan dengan tombol Send
    if (sendMessageBtn) {
        sendMessageBtn.addEventListener('click', sendMessageToAdmin);
    }

    // Mengirim pesan dengan Enter di input
    if (chatInput) {
