    ...
    <main class="flex-1 px-6 py-8 ml-64 bg-[#10172A] min-h-screen">
        <div class="max-w-5xl mx-auto space-y-6">
            <a href="{{ route('customer.produk') }}"
                class="text-sm text-white hover:underline"><i class="fa-solid fa-arrow-left"></i> View Product</a>
                    ...
                    <div class="flex flex-col justify-between">
                        ...
                        <div class="mt-6 flex items-center justify-end">
                            <div class="flex gap-3">
                                ...
                                @endauth
                                @guest
                                    <a href="{{ route('login') }}" class="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-500">
                ...
                <div class="space-y-6">
                    ...
                    <form id="comment-form" action="{{ route('comments.store', $product->id) }}" method="POST"
                        ...
                        <div class="w-10 h-10 rounded-full overflow-hidden">
                            <img src="{{ Auth::user()->profile_picture_url }}" alt="User Avatar" class="w-full h-full object-cover rounded-full" />
                        </div>
                    ...
                    <div class="space-y-4">
                        @forelse ($product->comments->sortByDesc('created_at') as $comment) {{-- Gunakan comments dari product yang sudah dimuat --}}
                        <div class="flex gap-3 bg-[#1F2A40] p-3 rounded-md" id="comment-item-{{ $comment->id }}">
                            {{-- Foto Profil Komentator --}}
                            <div class="w-10 h-10 rounded-full overflow-hidden flex-shrink-0">
                                <img src="{{ $comment->user->profile_picture_url ?? asset('img/default-profile.png') }}"
                                    alt="Commenter Avatar" class="w-full h-full object-cover rounded-full" />
                            </div>
                            <div class="flex-1">
                                <div class="flex items-center justify-between">
                                ...
                                <p class="text-sm text-gray-300 mt-1" id="comment-content-display-{{ $comment->id }}">{{ $comment->content }}</p>
                        ...
                        .then(response => {
                            if (!response.ok) {
                                return response.json().then(errorData => {
                                    throw new Error(errorData.message || 'Gagal mengubah status wishlist.');
                                });
                            }
