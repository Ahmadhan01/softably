...
<aside class="w-64 bg-[#1e293b] flex flex-col justify-between fixed top-0 left-0 h-full">
    <div>
        <div class="p-4 text-xl font-bold border-b border-gray-700">
            <img src="{{ asset('img/softably-baru.png') }}" alt="Logo" width="120px" />
        </div>
                ...
                <div class="relative w-10 h-10">
                    @php
                        $profile_picture = Auth::user()->profile_picture
                            ? url('profile/' . Auth::user()->profile_picture)
                            : asset('img/man.jpg');
                    @endphp
                    ...
                    <span
                        class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#1e293b] {{ Auth::user()->isOnline() ? 'bg-green-500' : 'bg-gray-400' }} z-10"></span>
                </div>
                <div>
