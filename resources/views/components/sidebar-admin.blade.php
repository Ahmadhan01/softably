        ...
        <div class="p-4 border-t border-gray-700">
            <div class="flex items-center space-x-3">
                <div class="relative w-10 h-10">
                    @php
                        $image = Auth::user()->image ? url('profile/' . Auth::user()->image) : asset('img/man.jpg');
                    @endphp

                    <div class="rounded-full overflow-hidden w-full h-full z-0">
                        <img src="{{ $image . '?t=' . time() }}" alt="photo" class="object-cover w-full h-full">
                    </div>

                    {{-- Badge status di atas avatar --}}
                    <span
                        class="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#1e293b]
