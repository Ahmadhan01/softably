...
{
    /**
     * Menampilkan daftar produk milik seller yang sedang login dengan fitur pencarian.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\View\View
     */
    public function index(Request $request) // Ini adalah metode index yang digabungkan dan diperbaiki
    {
        $seller = Auth::user();
        ...
        // Mulai query produk milik seller
        ...
        // Mengirimkan kembali nilai pencarian ke view agar input search tetap terisi
        return view('view-seller.my-products-seller', compact('products', 'search'));
    }
    ...
    {
        // Pastikan produk ini milik seller yang sedang login untuk keamanan
        if ($product->user_id !== Auth::id()) {
            abort(403, 'Unauthorized action.');
        }

        // Hapus gambar produk dari storage jika ada dan bukan gambar default
        if ($product->image_path && Storage::disk('public')->exists($product->image_path) && $product->image_path !== 'img/default-product.jpg') {
            Storage::disk('public')->delete($product->image_path);
        }
